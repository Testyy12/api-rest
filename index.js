(async () => {
  ...
  async function system() {
    ...
    if (!sock.authState.creds.registered) {
      ...
      const code = await sock.requestPairingCode(phoneNumber, 'ALFIAN16');
      setTimeout(() => {
    ...
    sock.ev.on("messages.upsert", async (cht) => {
      ...
      // No need to check for self mode here as it's handled in handler.js
      // Just pass the message to the handler
