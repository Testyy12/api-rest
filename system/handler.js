...
module.exports = async (m, sock, store) => {
  ...
  // Early return for bot messages, status broadcasts, and self mode
  if (m.isBot) return;
  ...
  // Implement self mode check early - only allow owner to continue if self mode is on
  ...
  for (let name in pg.plugins) {
    ...
    if (!plugin) continue; // Changed from return to continue to keep checking other plugins
    ...
    try {
      ...
      // Auto-read message based on settings
