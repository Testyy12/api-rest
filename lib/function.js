...
class Function {
  ...
  // Helper function to format OS names for display
   formatOSName(osSlug) {
    const osMap = {
      ...
      'fedora-36-x64': 'Fedora 36'
    };

    return osMap[osSlug] || osSlug;
  }

  // Helper function to format region names for display
   formatRegionName(regionSlug) {
    const regionMap = {
      'sgp1': 'Singapore (SGP1)',
      'blr1': 'Bangalore (BLR1)',
      'nyc1': 'New York (NYC1)',
    ...
    return regionMap[regionSlug] || regionSlug;
  }
  isUrl(url) {
