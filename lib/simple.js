const { makeWASocket, jidDecode, areJidsSameUser, generateWAMessageFromContent, generateWAMessage, prepareWAMessageMedia, proto, WA_DEFAULT_EPHEMERAL, downloadContentFromMessage, generateForwardMessageContent } = require("baileys");
...
module.exports = (connection, store) => {
  ...
  let sock = makeWASocket(connection);
  ...
  sock.sendButton = async (jid, array, quoted, json = {}) => {
    ...
    for (const data of array) {
      if (data.type === "reply") {
        ...
        result.push({
          buttonId: data.id || "list",
          buttonText: {
            displayText: data.displayText || "List"
          },
          nativeFlowInfo: {
            name: "single_select",
...
PHOTO;BASE64:${((await sock.getFile(await sock.profilePictureUrl(njid).catch((_) => ({}))) || {}).data?.toString("base64") || "")}
...
`.trim();
    ...
    if (jid.endsWith("@g.us")) {
      ...
      if (!(v.name || v.subject)) {
        return sock.groupMetadata(jid)
          .then(metadata => metadata || store?.fetchGroupMetadata(jid, sock.groupMetadata))
          .then(metadata => metadata || {})
          .then(metadata => metadata.name || metadata.subject || 
  ...
  sock.copyNForward = async (jid, message, forwardingScore = true, quoted, options = {}) => {
  ...
  // Helper function for file extension when not provided
  const extension = (mime) => {
    return mime.split('/')[1];
  };

  // Add function to send template messages
  ...
  // Add function to send polls
  sock.sendPoll = async (jid, name, options, quoted, pollOptions = {}) => {
    return sock.sendMessage(
      jid,
      {
        poll: {
          name,
          options,
          selectableOptionsCount: pollOptions.multiSelect ? options.length : 1,
          ...pollOptions
        }
      },
      {
  ...
  // Add function to send reactions to messages
  sock.sendReaction = async (jid, key, text) => {
    return sock.sendMessage(
      jid, 
      { 
        react: { 
  ...
  // Add function to get message info
  sock.getMessageInfo = async (jid, messageID) => {
  ...
  // Add serialization function reference
  sock.serializeM = (m) => {
